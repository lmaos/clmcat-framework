...
public class Base36 {
    ...
    public static String encode(long value) {
        if (value < 0) {
            throw new IllegalArgumentException("Base36不支持负数编码");
        }
        if (value == 0) {
    ...
    public static long decodeToLong(String input) {
        ...
        if (value.bitLength() > Long.SIZE - 1) {
            throw new IllegalArgumentException("Base36值超出long范围");
        }
    ...
    public static String encode(byte[] input) {
        if (input == null) {
            throw new IllegalArgumentException("Base36输入不能为空");
        }
        if (input.length == 0) {
            return "";
        }
    ...
    public static BigInteger decodeToBigInteger(String input) {
        ...
        for (int i = 0; i < normalized.length(); i++) {
            ...
            if (digit < 0) {
                throw new IllegalArgumentException("非法Base36字符: " + normalized.charAt(i));
            }
