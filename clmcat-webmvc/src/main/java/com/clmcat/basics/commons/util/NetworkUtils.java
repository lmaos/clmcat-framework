...
public final class NetworkUtils {
    ...
    public static List<String> getLocalIpAddresses(String preferredInterface) {
        ...
        try {
            ...
            while (interfaces.hasMoreElements()) {
                ...
                String niDisplayName = ni.getDisplayName();
                boolean virtual = isVirtualInterface(niName) || isVirtualInterface(niDisplayName);

                Enumeration<InetAddress> inetAddresses = ni.getInetAddresses();
                while (inetAddresses.hasMoreElements()) {
                    InetAddress addr = inetAddresses.nextElement();
                    if (addr.isLoopbackAddress() || addr.isLinkLocalAddress()) continue;
                    entries.add(new AddressEntry(addr, virtual, niName, niDisplayName));
                }
            }
        ...
        return Collections.unmodifiableList(sortAddressTexts(entries, preferredInterface));
    }

    static List<String> sortAddressTexts(List<AddressEntry> entries, String preferredInterface) {
        if (entries == null || entries.isEmpty()) {
            return Collections.emptyList();
        }
        List<AddressEntry> sortedEntries = new ArrayList<>(entries);
        String normalizedPreferredInterface = normalizeInterfaceName(preferredInterface);

        // 排序：1. 优先接口  2. 非虚拟优先  3. 地址优先级  4. 字典序
        sortedEntries.sort(Comparator
                .comparingInt((AddressEntry entry) -> preferredInterfacePriority(entry, normalizedPreferredInterface))
                .thenComparingInt(entry -> entry.isVirtual ? 1 : 0)
                .thenComparingInt(entry -> addressPriority(entry.address))
                .thenComparing(entry -> entry.address.getHostAddress()));

        List<String> result = new ArrayList<>(sortedEntries.size());
    ...
    private static int preferredInterfacePriority(AddressEntry entry, String normalizedPreferredInterface) {
        if (normalizedPreferredInterface == null) {
            return 0;
        }
        return matchesInterface(normalizedPreferredInterface, entry.interfaceName)
                || matchesInterface(normalizedPreferredInterface, entry.interfaceDisplayName) ? 0 : 1;
    }

    private static boolean matchesInterface(String normalizedPreferredInterface, String interfaceName) {
        String normalizedInterfaceName = normalizeInterfaceName(interfaceName);
        if (normalizedInterfaceName == null) {
            return false;
        }
        if (normalizedPreferredInterface.equals(normalizedInterfaceName)) {
            return true;
        }
        return baseInterfaceName(normalizedPreferredInterface).equals(baseInterfaceName(normalizedInterfaceName));
    }
    ...
    private static String normalizeInterfaceName(String interfaceName) {
        ...
        return interfaceName.trim().toLowerCase(Locale.ROOT);
    }

    private static String baseInterfaceName(String interfaceName) {
        int aliasIndex = interfaceName.indexOf(':');
        return aliasIndex >= 0 ? interfaceName.substring(0, aliasIndex) : interfaceName;
    }
    ...
    static class AddressEntry {
        ...
        AddressEntry(InetAddress address, boolean isVirtual, String interfaceName, String interfaceDisplayName) {
